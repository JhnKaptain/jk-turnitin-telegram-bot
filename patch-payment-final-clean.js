...
fs.writeFileSync("bot.js.before-payment-final-clean", code, "utf8");
...
const anchor = 'const INTERNATIONAL_BANK_ACCOUNT_NAME = String(process.env.INTERNATIONAL_BANK_ACCOUNT_NAME || "").trim();';
...
const TZ_OTHER_CURRENCY = String(process.env.TZ_OTHER_CURRENCY || INTERNATIONAL_CURRENCY || "KES")
  ...
  code = code.replace('readIntEnv("TZ_OTHER_PROOF_WAIT_MINUTES", 3)', 'readIntEnv("TZ_OTHER_PROOF_WAIT_MINUTES", 2)');
}
...
const oldPhoto1 = `if (sub && sub.stage === STAGE_WAIT_PAYMENT) {
...
const newPhoto = `if (sub && sub.stage === STAGE_WAIT_PAYMENT) {
...
if (code.includes(oldPhoto1)) code = code.replace(oldPhoto1, newPhoto);

const oldText1 = `if (sub && sub.stage === STAGE_WAIT_PAYMENT) {
    if (sub.paymentMethod === "INTERNATIONAL") {
...
const newText = `if (sub && sub.stage === STAGE_WAIT_PAYMENT) {
    if (sub.paymentMethod === "INTERNATIONAL" || sub.paymentMethod === "KENYAN_BANK") {
      await handleInternationalPaymentProofText(ctx, sub, text);
    } else if (sub.paymentMethod === "TZ_OTHER") {
...
if (code.includes(oldText1)) code = code.replace(oldText1, newText);
...
if (!code.includes('bot.action("PAYMENT_METHOD_TZ_OTHER"')) {
  code = code.replace(actionAnchor, `
...
code = code.replaceAll("International payment enabled:", "Kenyan bank payment enabled:");
code = code.replaceAll("International Payment", "Kenyan Bank Payment");
code = code.replaceAll("International payment", "Kenyan bank payment");

fs.writeFileSync(path, code, "utf8");
console.log("Final payment restructure applied.");

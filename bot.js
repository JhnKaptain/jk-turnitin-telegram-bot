...
function detectTurnitinReportType({ fileName, caption, text }) {
  ...
  const aiPatterns = [
    { pattern: /\bai writing report\b/i, weight: 6 },
    { pattern: /\bturnitin ai\b/i, weight: 5 },
    { pattern: /\bai writing\b/i, weight: 5 },
    { pattern: /\bai detection\b/i, weight: 5 },
    { pattern: /\bai generated\b/i, weight: 4 },
    { pattern: /\bai-generated\b/i, weight: 4 },
    { pattern: /\bgenerated by ai\b/i, weight: 4 },
    { pattern: /\bai score\b/i, weight: 4 },
    { pattern: /\bhuman-written\b/i, weight: 3 },
    { pattern: /\bhuman written\b/i, weight: 3 },
    { pattern: /\bqualifying text\b/i, weight: 3 },
    { pattern: /\bchatgpt\b/i, weight: 2 }
  ];

  const plagPatterns = [
    { pattern: /\bsimilarity report\b/i, weight: 6 },
    { pattern: /\bturnitin similarity\b/i, weight: 5 },
    { pattern: /\boverall similarity\b/i, weight: 5 },
    { pattern: /\bsimilarity index\b/i, weight: 5 },
    { pattern: /\bmatched sources\b/i, weight: 4 },
    { pattern: /\bmatch overview\b/i, weight: 4 },
    { pattern: /\bsource overview\b/i, weight: 4 },
    { pattern: /\binternet sources\b/i, weight: 3 },
    { pattern: /\bpublications\b/i, weight: 3 },
    { pattern: /\bstudent papers\b/i, weight: 3 },
    { pattern: /\bexcluded sources\b/i, weight: 2 }
  ];

  const aiScore = countPatternMatches(combined, aiPatterns);
  const plagScore = countPatternMatches(combined, plagPatterns);

  if (aiScore >= 5 && aiScore >= plagScore + 2) {
    return {
      kind: "AI",
      prefix: "AI",
      confidence: "high",
  ...
  if (plagScore >= 5 && plagScore >= aiScore + 2) {
    return {
      kind: "PLAG",
      prefix: "Plag",
      confidence: "high",
...
bot.on("document", async (ctx) => {
  ...
  if (user.id === ADMIN_ID) {
    ...
    try {
      await ctx.reply("🔍 Checking document type before sending...");
      ...
      const routeLabel = sentWithPrefix
        ? `renamed as ${finalFileName}`
        : "normal route, no prefix added";

      const detectionNote = detectionError
        ? `\nDetection note: ${safeText(detectionError?.message || detectionError)}`
        : "";

      await ctx.reply(
        `✅ Document sent to ${target.userId}\nRoute: ${routeLabel}\nAI score: ${detection.aiScore}\nPlag score: ${detection.plagScore}${detectionNote}`
      );
    } catch (err) {
      await ctx.reply("❌ Failed: " + (err?.message || err));
    } finally {
